                ...
                for (int j = 0; j < DTSMSTypes.Rows.Count; j++)
                {
                    ...
                    for (int i = 0; i < DTDabwalfarms.Rows.Count; i++)
                    {
                        ...
                        if (MessageType == "forecast" && DateTime.Now.Hour >= 10 && DateTime.Now.Hour <= 24)
                        {
                            ...
                            if (databaserainalert == "")
                            {
                                //Insert Data to Data base
                                execQuery("Insert into mfi.rain_alert_checkstatus (FarmID,RainAlertStatus,LastCheckDate) Values ('" + FarmID + "','" + rainalert + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')");
                                if (rainalert.ToLower() == "yes")
                                    flagoutturnforecast = "yes";
                            }
                            else
                            {

                                if (rainalert.ToLower() != databaserainalert.ToLower())
                                    flagoutturnforecast = "yes";
                                execQuery("update mfi.rain_alert_checkstatus set RainAlertStatus='" + rainalert + "',LastCheckDate='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where FarmID='" + FarmID + "'");
                            }
                        ...
                        else if (forecastdayflag.ToLower() == "no" && flagoutturnforecast.ToLower()=="no")
                            continue;
